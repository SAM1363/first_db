...
SELECT * FROM project_uses_tech put
INNER JOIN project p ON put.project_id = p.id 
-- INNER JOIN tech t ON put.tech_id = t.id
-- WHERE p.name ilike '%website%'; 
LEFT OUTER JOIN tech t on put.tech_id = t.id
where t.name !=== p;
